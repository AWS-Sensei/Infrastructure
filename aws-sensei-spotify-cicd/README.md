...
This repository/folder contains a **modular, multi-file AWS SAM project** for the AWS Sensei Spotify CI/CD Infrastructure.\
Infrastructure is split into multiple CloudFormation templates that are combined through a single `master.yaml`.\
This CodePipeline deploys for the AWS Sensei Spotify Backend the **SAM infrastructure** automatically.
...
``` bash
sam deploy --stack-name AWS-Sensei-Spotify-CICD --resolve-s3 --capabilities CAPABILITY_NAMED_IAM
```
...
1. GitHub source
2. Lambda Package
3. InfraDeploy via SAM/CloudFormation
